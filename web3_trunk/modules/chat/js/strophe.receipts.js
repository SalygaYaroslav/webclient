...
//        this.resendMessage(id);
    ...
    _processReceipt: function (msg) {
        var id = msg.getAttribute('id'),
                ...
                body = $(msg.getElementsByTagName('body')).text();
        // check for request in message
        if (req.length > 0) {
            $(document).trigger('incoming.receipts', {from: from, to: this._conn.jid, id: id, body: body});
            // send receipt
        ...
        if (rec.length > 0) {
            ...
            $(document).trigger('delivery.receipts', msg);
