TreeForm.Image = function (parent, data) {
    /** private */
    let local = {
        parent: parent,
    ...
    let self = {
        ...
        render: function () {
            ...
            local.work.template = $(Template.render('treeform', 'elements/image', local.template_options));
            $('input[type=file]', local.work.template).each(function (i) {
                Upload.setUploadByInput($(this), local.parent.getUploadId(), Authorization.getCurrentUser().getFileTask(), function (file, name, e) {
                    self.appendImage(i, file, name, e);
                });
            });
        ...
        getData: function () {
            let data = [];
            $('.tform-image-full', local.work.template).each(function () {
                data.push($(this).val());
            });
            return {[local.options.id]: data.clearEmpty().join(local.options.split || ',')};
        },
        appendImage: function (index, file, name) {
            let link = $('.tform-image-full', local.work.template).eq(index);
            ...
            $('.tform-image-value', local.work.template).eq(index).val(file.name);
            link.val(file.name + '\n' + name + '.' + file.extra.extension + '\n' + name);
        },
        deleteImage: function (delete_button) {
            let index = $('.tform-image-delete', local.work.template).index(delete_button);
            let element = $('.tform-element', local.work.template).eq(index);
            let link = $('.tform-image-full', element);
            if (local.work.need_load) {
                Upload.removeFileFromQueue(local.parent.getUploadId(), link.val());
                local.work.need_load = false;
                link.val('');
                $('.tform-image-value', element).val('');
            } else {
                link.val('');
                $('.tform-image-value', element).val('');
            }
