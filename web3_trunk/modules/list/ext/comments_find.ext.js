 ...
 * @param params
 * @returns {{load: load, appendComment: appendComment, getCommentById: getCommentById, addLike: addLike, getListComments: getListComments, needUpdate: needUpdate}}
 * @constructor
 */
List.CommentsFind = function (params) {
    /** private */
    let local = {
        task_id: null,
        task: null,
    ...
    let loadFromServer = function (callback) {
        ...
        let comments = [];
        local.load_now = true;
        Request.findComments(params, function (request_data) {
    ...
    local.task = Dataset.get('task',local.task_id);
    local.format_comment = function (object) {
        return object;
    };
    loadLikes();
    /** public */
