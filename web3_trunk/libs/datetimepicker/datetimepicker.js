...
(function ($) {
    ...
    var default_options = {
        i18n: {
            ...
            tr: {// Turkish
                months: [
                    "Ocak", "Subat", "Mart", "Nisan", "May?s", "Haziran", "Temmuz", "Agustos", "Eylul", "Ekim", "Kas?m", "Aral?k"
                ],
                dayOfWeek: [
            ...
            createDateTimePicker = function (input) {
                ...
                if (options.appendTo) {
                    if (typeof options.appendTo == "string") {
                        $(options.appendTo).append(datetimepicker);
                    } else {
                        options.appendTo.append(datetimepicker);
                    }
                }
                ...
                mounth_picker
                    ...
                    .on('mousedown.xdsoft', function () {
                        ...
                        if (options.onTodayButtonClick && options.onTodayButtonClick.call) {
                            options.onTodayButtonClick.call(datetimepicker, _xdsoft_datetime.currentTime, datetimepicker.data('input'));
                        }
                ...
                datetimepicker
                    .on('xchange.xdsoft', function (event) {
                        ...
                        xchangeTimer = setTimeout(function () {
                            ...
                            var ww = 6, d, y, m, w, classes = [], customDateSettings, newRow = true;
                            while ((i < _xdsoft_datetime.currentTime.countDaysInMonth() || start.getDay() != options.dayOfWeekStart || _xdsoft_datetime.currentTime.getMonth() == start.getMonth()) || ((options.weekSix) ? ww > 0 : false)) {
                                classes = [];
                                i++;
                                d = start.getDate();
                                y = start.getFullYear();
                                ...
                                if (newRow) {
                                    ...
                                    ww--;
                                    if (options.weeks) {
                                        table += '<th data-month="' + m + '" data-year="' + y + '" data-start="' + d + '">' + w + '</th>';
                                    }
                                }
                ...
                calendar
                    .on('click.xdsoft', 'th', function (xdevent) {
                        xdevent.stopPropagation();
                        timerclick++;
                        if (options.onSelectWeek) {
                            options.onSelectWeek(xdevent, this);
                        }
                        setTimeout(function () {
                            timerclick = 0;
                        }, 200);
                ...
                var setPos = function () {
                    ...
                    if (options.fixed) {
                        ...
                        if (eval($(datetimepicker).height() + top) > $(window).height()) {
                            top -= $(datetimepicker).height();
                        }
                        left -= $(window).scrollLeft();
                        if (eval($(datetimepicker).width() + left) > eval($(window).width() - 15)) {
                            left -= eval($(window).width() - eval($(datetimepicker).width() + left - 15));
                        }
                        position = "fixed";
                    } else {
...
Date.patterns = {
    ...
    RussianDateTimePattern: "d.m.Y H:i"
};